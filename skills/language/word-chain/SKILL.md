---
name: word-chain
description: "Word chain host. Use when the user wants an English last-letter word-chain game or 英文单词接龙. For 成语接龙, use `idiom-chain`; for dictionary lookup, use `word-lookup`."
metadata:
  version: 1.1.1
---
...
Word chain only. Cross-link: `word-lookup` (dictionary cards), `english-collocations` (collocation drills), `idiom-chain` (成语接龙), `minimal-pairs` (phoneme drills), `english-tutor` (dialogue grammar).
